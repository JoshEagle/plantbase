...
col2.image('plantbase_logo.png', width=200, output_format='png') #exchange logo for something else?
...
#----------------two pictures under  the plant Genus---------------
    ...
    if st.button(f'Yes, my plant looks like {pred1}!'):
        ...
        for i in range(5):
            ...
            if int(weather['min_temp'][i].split()[0]) <= 2:
    ...
    with col1:
        st.image(pred2_img, use_column_width=True)
        st.subheader(pred2)
        if st.button(f'My plant actually looks like {pred2}.'):
            plant_name = pred2
    with col2:
        st.image(pred3_img, use_column_width=True)
        st.subheader(pred3)
